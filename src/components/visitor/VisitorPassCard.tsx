...
export const VisitorPassCard: React.FC<VisitorPassCardProps> = ({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-xl hover:shadow-2xl transition-all duration-300">
      {/* Top Banner Accent */}
      <div className="h-2 w-full bg-gradient-to-r from-emerald-500 via-teal-500 to-indigo-500" />

      {/* Header */}
      <div className="p-6 pb-4 border-b border-slate-200/80 dark:border-slate-800">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-lg font-bold tracking-tight text-slate-900 dark:text-slate-100 flex items-center gap-2">
                VISITOR PASS
                <span className="text-sm font-normal text-slate-400 dark:text-slate-500">| Thẻ Khách</span>
              </h3>
            </div>
          </div>
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-mono font-medium text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100 transition-colors group"
            >
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
        {/* QR Code Container */}
        <div className="md:col-span-5 flex flex-col items-center justify-center p-4 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
        ...
        <div className="md:col-span-7 space-y-4">
          ...
          {pass.note && (
            <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200/80 dark:border-slate-700/80 flex items-start gap-2 text-xs">
              <FileText className="h-4 w-4 text-slate-400 shrink-0 mt-0.5" />
              <span className="text-slate-600 dark:text-slate-300 italic line-clamp-2">
          ...
          {(pass.checkInAt || pass.checkOutAt) && (
            <div className="pt-2 border-t border-slate-200/80 dark:border-slate-800 flex flex-wrap items-center gap-4 text-xs">
              {pass.checkInAt && (
      ...
      {showActions && pass.status === 'PENDING' && onCancel && (
        <div className="px-6 py-3.5 bg-slate-50/80 dark:bg-slate-800/50 border-t border-slate-200/80 dark:border-slate-800 flex items-center justify-between">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-medium text-rose-500 hover:text-rose-600 dark:hover:text-rose-400 disabled:opacity-50 transition-colors cursor-pointer"
          >
